...
export function ImageBlurTool() {
  ...
  const run = useCallback(async () => {
    ...
    const items: ResultItem[] = res.map((r: any) => ({
      name: r.name, url: resultBlobUrl(r.mime, r.bytes), size: r.size, detail: `${r.width}Ã—${r.height}px | radius ${effectiveRadius}px`,
    }))
    setResult(items)
  }, [files, effectiveRadius, worker])

  const reset = useCallback(() => { setFiles([]); setResult(null); releaseResultUrls(); setPreset('medium'); setRadius(5) }, [])
