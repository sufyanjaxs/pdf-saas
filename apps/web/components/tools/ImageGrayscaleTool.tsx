...
export function ImageGrayscaleTool() {
  ...
  const run = useCallback(async () => {
    ...
    const items: ResultItem[] = res.map((r: any) => ({
      name: r.name, url: resultBlobUrl(r.mime, r.bytes), size: r.size, detail: `${r.width}×${r.height}px | ${intensity}% grayscale`,
    }))
    setResult(items)
  }, [files, intensity, worker])

  const reset = useCallback(() => { setFiles([]); setResult(null); setIntensity(100) }, [])
